...
@Component({
  selector: 'app-accents-map',
  templateUrl: './accents-map.component.html',
  styleUrls: ['./accents-map.component.scss']
})
export class AccentsMapComponent implements OnDestroy {
  ...
  private selectFeature(featureLayer) {
    if (featureLayer !== this.selectedCountry) {
      ...
      this.select.emit(featureLayer != null ? featureLayer.feature.properties.name : 'World');
    }
  }
